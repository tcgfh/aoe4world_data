# Bedouin Swordsman

Master Swordsman who excels in melee combat

- **+** Bonus vs Melee units
- **−** Weak to ranged units
- Requires the Trade Wing - Bazaar

![Bedouin Swordsman](https://data.aoe4world.com/images/units/bedouin-swordsman-1.png)

## Overview

| Field | Value |
| --- | --- |
| ID | `bedouin-swordsman` |
| Type | Unit |
| Civilization | Ayyubids (ay) |
| Unique | Yes |
| Display class | Light Melee Infantry |
| Minimum age | I (Dark) |
| Produced by | — |

**Classes:** annihilation_condition, find_non_siege_land_military, formational, human, included_by_military_hotkeys, infantry, infantry_light, land_military, light_melee_infantry, melee, melee_infantry, military, militia, torch_thrower

## Variations

### Bedouin Swordsman (Age I)

| Field | Value |
| --- | --- |
| ID | `bedouin-swordsman-1` |
| pbgid | 2146044 |
| attribName | `unit_pirate_abb_ha_01` |
| Hitpoints | 160 |
| Cost | 60 food |
| Build time | 0s |
| Population | 1 |
| Produced by | — |
| Movement speed | 1.25 |
| Armor | none |
| Icon | https://data.aoe4world.com/images/units/bedouin-swordsman-1.png |

**Short Sword** — melee (`weapon_pirate_abb_ha_01`, pbgid 2146045)

| Damage | Attack speed | Range (min–max) |
| --- | --- | --- |
| 11 | 1.125 | 0–0.295 |

Modifiers:

- meleeAttack +3 vs melee (passive)

Durations: aim 0, windup 0.5, attack 0.125, winddown 0.5, reload 0, setup 0, teardown 0, cooldown 0

**Torch** — fire (`weapon_torch`, pbgid 123518)

| Damage | Attack speed | Range (min–max) |
| --- | --- | --- |
| 10 | 2.125 | 0–1.25 |

Durations: aim 0, windup 0.75, attack 0.125, winddown 0, reload 0, setup 0, teardown 0, cooldown 1.25

**Sight:** base 21.6, line 36, height 10 (inner radius 12 / height 10, outer radius 36 / height -15)

---

Source: autogenerated AoE4 data — see https://data.aoe4world.com/. Regenerate with `python3 scripts/units_to_markdown.py`.
